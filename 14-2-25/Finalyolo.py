...
import datetime
import json
from Finalvision import analyze_image 
...
classes_to_detect = [0, 24, 25, 26, 32, 34, 39, 41, 45, 56, 63, 64, 66, 67, 73, 76]
...
while cap.isOpened():
    ...
    for i in range(len(boxes)):
        ...
        # Construct live text with details
        ...
        if int(class_ids[i]) != 0:
            ...
            snapshot_text = f"{labels[i]} | ID: {int(track_ids[i])} | Index: {int(class_ids[i])} | {confidences[i]:.2f}"
            (text_width, text_height), baseline = cv2.getTextSize(snapshot_text, cv2.FONT_HERSHEY_SIMPLEX, 0.6, 2)
            cv2.rectangle(snapshot_frame, (x1, y1 - text_height - baseline), (x1 + text_width, y1), (0, 0, 0), -1)
            cv2.putText(snapshot_frame, snapshot_text, (x1, y1 - baseline), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
    ...
    if new_snapshot:
        ...
        # Build detected objects list (if needed for further processing)
        detected_objects = []
        for i in range(len(boxes)):
            obj = {
                "label": labels[i],
                "box": boxes[i].tolist(),
                "properties": {}  # you can add further properties here.
            }
            detected_objects.append(obj)
        
        # Call the updated analyze_image from visiontest.py
        analysis_result = analyze_image(output_path)
        print("Analysis result:", analysis_result)
        
        # Automatically set analysis timestamp using the device's date/time
        analysis_timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        # Set unique id as increment of previous snapshot (starting from 1)
        unique_id = snapshot_count
        
        # Append the analysis results to a CSV file in the format:
        # AnalysisTime, PersonUniqueID, PersonName, Objects
        csv_file = os.path.join(results_folder, "analysis_results.csv")
        file_exists = os.path.isfile(csv_file)
        with open(csv_file, mode="a", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            if not file_exists:
                writer.writerow(["AnalysisTime", "PersonUniqueID", "PersonName", "Objects"])
            try:
                ...
                analysis_json = [{"person": "unknown", "objects": []}]
            
            for entry in analysis_json:
                person_name = entry.get("person", "unknown")
                # Look up the unique ID or set to -1 for unknown persons
                person_id = known_person_ids.get(person_name, -1) if person_name.lower() != "unknown" else -1
                objects = entry.get("objects", [])
                writer.writerow([analysis_timestamp, person_id, person_name, json.dumps(objects)])
