document.addEventListener('DOMContentLoaded', (event) => {
    //---------------------------------------------------------------------
    //fetch faculty db data
    var Username;
    const id = localStorage.getItem('ruserId');
    (async function() {
      try {
        const response = await fetch(`http://localhost:3000/api/facultydata/${id}`);
    ...
    document.getElementById("Ktuid").innerText = facid;
    ...
    fetch(`http://localhost:3000/api/facultyreqitems?usertype=faculty&userId=${ruserId}`)
    ...
    document.querySelector('.bw-form').addEventListener('submit', async function(event) {
        ...
        try {
            const response = await fetch(`http://localhost:3000/facultyupdate/${userId}`, {
                method: 'PUT',
                body: formData
            });
    ...
    //------------------------------------------------------------------
    fetch(`http://localhost:3000/api/facultydelitems/${data._id}`, {
      method: 'DELETE',
    })
    .then(response => response.json())
    .then(data => {
    ...
    fetch(`http://localhost:3000/api/facultyreporteditems?usertype=faculty&userId=${repuserId}`)
    .then(response => response.json())
    .then(data => {
      // Loop through the data and create a list for each item
