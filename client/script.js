...
window.onload = function() {
    var mainLogo = document.getElementById('maindivlogo'); // Replace with your main logo's id
    if (mainLogo) {
        mainLogo.addEventListener('mouseover', function() {
            if (preloader.style.display === 'none') {
                gsap.to(cursor, speed, {scale: 16}); // Adjust scale as needed
            }
        });
...
var typingTexts = {
    25: 'ITEMS ARE NEVER REUNITED WITH THEIR OWNERS',
    50: 'STUDENTS LOSE SOMETHING ON AVERAGE A YEAR.',
    60: 'PEOPLE LOSE SOMETHING IN A YEAR.',
    90: 'WHO FIND ITEMS MAKE AN EFFORT TO RETURN THEM.',
...
function typeText() {
    if (typingIndex < typingText.length) {
        ...
        typingIndex++;
        typingTimeout = setTimeout(typeText, Math.random() * 40);
    } else {
        setTimeout(function() {
            if (progress < 100) {
